	...
	bool SendToPeer(const char* name, const char* text)
	{
		...
		while(peer_socket_map.end() != iter)
		{
			if(strcmp(name, iter->first.c_str()) == 0)
			{
				...
				int len = sendto(sock, text, strlen(text)>102399?102399:strlen(text), 0, (const sockaddr*)&addr, sizeof(addr));
				DWORD err = GetLastError();
				cout<<"len sent:"<<len<<";errno:"<<err<<endl;
				return true;
			}
			iter++;
		}
	...
	void HandleInUseFD(long msec)
	{
		if(fds_inuse.size() > 0)
		{
			...
			while(iter != fds_inuse.end())
			{
				...
				getsockname(*iter, (sockaddr*)&myaddr, &size);
				cout<<"sock port;"<<ntohs(myaddr.sin_port)<<endl;
				int ret  = sendto(*iter, "fuck", 4, 0, (const sockaddr*)&myaddr, size);
				ret = GetLastError();
				cout<<"test send len:"<<ret<<endl;*/
				iter++;
			}
			int ret = select(maxFd+1, &set, NULL, &set_error, &tv);
			if(ret < 0)
